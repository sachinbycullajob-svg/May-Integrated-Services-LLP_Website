      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`mb-16 p-6 sm:p-8 rounded-md border relative overflow-hidden ${
          themeMode === 'dark'
            ...
            : 'bg-beige-50 border-beige-300 shadow-md'
        }`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className={`text-xl font-bold ${themeMode === 'dark' ? 'text-white' : 'text-maroon-950'}`}>
              ...
              <p className={`text-xs sm:text-sm leading-relaxed ${
                ...
                May Integrated Services LLP is a legally incorporated Limited Liability Partnership in India under the Ministry of Corporate Affairs (MCA). Although formally incorporated in May 2026, we bring over 6 years of proven hands-on international e-commerce experience.
              </p>

              {/* Legal Info Table */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 text-xs">
                <div className={`p-4 rounded-md border ${
                  themeMode === 'dark' ? 'bg-maroon-950 border-maroon-800' : 'bg-beige-100 border-beige-300'
                }`}>
                  <span className={`block text-[10px] uppercase font-bold tracking-wider mb-1 ${
                    themeMode === 'dark' ? 'text-gray-500' : 'text-gray-400'
                  }`}>LLPIN</span>
                  <span className="text-maroon-600 dark:text-maroon-500 font-mono font-bold text-sm">{COMPANY_INFO.llpin}</span>
                </div>
                <div className={`p-4 rounded-md border ${
                  themeMode === 'dark' ? 'bg-maroon-950 border-maroon-800' : 'bg-beige-100 border-beige-300'
                }`}>
                  ...
                  <span className={`font-semibold ${themeMode === 'dark' ? 'text-white' : 'text-maroon-950'}`}>{COMPANY_INFO.registrationDate}</span>
                </div>
                <div className={`p-4 rounded-md border ${
                  themeMode === 'dark' ? 'bg-maroon-950 border-maroon-800' : 'bg-beige-100 border-beige-300'
                }`}>
                  ...
                  <span className={`font-semibold ${themeMode === 'dark' ? 'text-white' : 'text-maroon-950'}`}>ROC Mumbai, MH</span>
                </div>
              </div>
              ...
              <div className="pt-2">
                <div className={`p-4 rounded-md border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs ${
                  themeMode === 'dark' ? 'bg-maroon-950 border-maroon-800' : 'bg-beige-50 border-beige-300'
                }`}>
                  <div>
                    <span className={`font-bold block mb-1 ${themeMode === 'dark' ? 'text-beige-200' : 'text-maroon-800'}`}>Registered Office Address:</span>
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-md font-semibold transition-colors flex items-center space-x-1.5 whitespace-nowrap border ${
                      themeMode === 'dark' ? 'bg-maroon-900 hover:bg-maroon-800 border-maroon-700 text-beige-200' : 'bg-beige-100 hover:bg-beige-200 border-beige-300 text-maroon-800'
                    }`}
                  >
              ...
              {COMPANY_INFO.partners.map((partner, idx) => (
                <div key={idx} className={`p-4 rounded-md border space-y-1 ${
                  ...
                  <div className="flex items-center justify-between">
                    <span className={`font-bold text-sm ${themeMode === 'dark' ? 'text-white' : 'text-maroon-950'}`}>{partner.name}</span>
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded-sm font-bold border ${
                      themeMode === 'dark' ? 'bg-maroon-900 text-gray-400 border-maroon-700' : 'bg-beige-50 text-gray-500 border-beige-300'
                    }`}>
                      DIN: {partner.din}
                    </span>
                  </div>
                  <p className="text-xs text-maroon-600 dark:text-maroon-500 font-semibold">{partner.role}</p>
