    ...
    <section id="careers" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <h2 className={`text-3xl sm:text-4xl font-extrabold tracking-tight ${
          ...
          Build the Future of <span className="text-maroon-400">Global E-Commerce</span>
        </h2>
        <p className="mt-3 text-sm sm:text-base text-slate-400 leading-relaxed">
          Join us as e-commerce specialists, data analysts, and vibe coders at our Mumbai command center. Applications are delivered directly to <span className="text-maroon-400 font-semibold">{COMPANY_INFO.email}</span>.
        </p>
      </div>
        ...
        {JOB_OPENINGS.map((job) => (
          <div
            ...
            className={`p-7 rounded-3xl border flex flex-col justify-between transition-all duration-300 ${
              themeMode === 'dark'
                ? 'bg-slate-900/90 border-slate-800 hover:border-maroon-500/40'
                : 'bg-beige-50 border-slate-200 shadow-xl hover:border-maroon-400'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-maroon-500/15 text-maroon-400 border border-maroon-500/20">
                  {job.type}
                </span>
                <span className="text-xs text-slate-400 flex items-center">
                  <MapPin className="w-3.5 h-3.5 mr-1 text-maroon-400" /> {job.location}
                </span>
              </div>
              ...
              <p className="text-xs font-semibold text-maroon-400 mb-3">{job.department}</p>
            ...
            <div>
              <button
                ...
                className="w-full py-3 rounded-2xl bg-maroon-500 hover:bg-maroon-400 text-white font-bold text-xs shadow-lg shadow-maroon-500/20 transition-all flex items-center justify-center space-x-2"
              >
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <div className={`w-full max-w-lg p-6 sm:p-8 rounded-3xl border shadow-2xl relative ${
            ...
            <p className="text-xs font-bold text-maroon-400 mb-4">{selectedJob.department} • {selectedJob.location}</p>
            ...
            {isApplied ? (
              <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-center space-y-2">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Thank you {applicantName}. Your application has been sent to <strong>{COMPANY_INFO.email}</strong> and saved to our Google Sheet tab "Job Applications". HR will reach out shortly.
                </p>
              </div>
            ) : (
              <form onSubmit={handleApply} className="space-y-4">
                ...
                <ul className="space-y-1.5 text-xs text-slate-300 mb-4">
                  {selectedJob.requirements.map((r, i) => (
                    <li key={i} className="flex items-center space-x-2">
                      <CheckCircle2 className="w-3.5 h-3.5 text-maroon-400 shrink-0" />
                      <span>{r}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-maroon-500"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-maroon-500 to-blue-600 text-white font-bold text-xs shadow-xl flex items-center justify-center space-x-2 disabled:opacity-50"
                >
