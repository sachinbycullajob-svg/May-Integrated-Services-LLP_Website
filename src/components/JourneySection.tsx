...
export const JourneySection: React.FC<JourneySectionProps> = ({ themeMode }) => {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      default: return <Sparkles className="w-5 h-5 text-maroon-400" />;
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className={`text-3xl sm:text-4xl font-extrabold tracking-tight ${
            ...
            Our Strategic <span className="text-maroon-400">Growth Journey</span>
          </h2>
            ...
            return (
              <button
                ...
                className={`px-5 py-2.5 rounded-2xl font-bold text-sm transition-all duration-300 flex items-center space-x-2 ${
                  isSelected
                    ? 'bg-gradient-to-r from-maroon-500 to-blue-600 text-white shadow-lg shadow-maroon-500/30 scale-105'
                    : themeMode === 'dark'
              ...
              >
                <span>{item.year}</span>
                {isSelected && <span className="w-2 h-2 rounded-full bg-beige-50 animate-pulse" />}
              </button>
            );
          })}
        </div>
        ...
        <div className="relative border-l-2 border-maroon-500/30 ml-4 sm:ml-8 lg:ml-12 pl-6 sm:pl-10 space-y-12">
            ...
            return (
              ...
              >
                ...
                <div className={`absolute -left-[35px] sm:-left-[51px] top-0 w-8 sm:w-10 h-8 sm:h-10 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                  isHighlight
                    ? 'bg-maroon-500 text-white ring-4 ring-maroon-500/30 shadow-lg shadow-maroon-500/40 scale-110'
                    : 'bg-slate-900 text-maroon-400 border border-slate-700'
                }`}>
                ...
                <Card3DTilt>
                  <div className={`p-6 sm:p-8 rounded-3xl border transition-all duration-300 ${
                    isHighlight
                      ? themeMode === 'dark'
                        ? 'bg-slate-900/90 border-maroon-500/50 shadow-2xl shadow-maroon-500/10 ring-1 ring-maroon-500/30'
                        : 'bg-beige-50 border-maroon-400 shadow-xl'
                      : themeMode === 'dark'
                        ? 'bg-slate-900/60 border-slate-800'
                        : 'bg-beige-50/80 border-slate-200 shadow-sm'
                  }`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                      <div>
                        ...
                        <p className="text-xs text-maroon-400 font-semibold">{milestone.subtitle}</p>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {milestone.brandsAdded.map((b, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 rounded-xl text-xs font-semibold bg-maroon-500/10 text-sky-300 border border-maroon-500/20"
                          >
                            + {b}
                          </span>
                        ))}
